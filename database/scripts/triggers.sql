...
CREATE OR REPLACE TRIGGER check_whether_apt_is_free
    ...
    ON BOOKINGS
    FOR EACH ROW
    when (new.START_DATE != old.START_DATE OR new.END_DATE != old.END_DATE or old.END_DATE is null)
DECLARE
...
CREATE OR REPLACE TRIGGER check_max_people_per_apartment
    ...
    ON BOOKINGS
    FOR EACH ROW
    WHEN (new.NO_PEOPLE != old.NO_PEOPLE or new.APARTMENT_ID != old.APARTMENT_ID)
DECLARE
    v_max_people number;
BEGIN
    SELECT MAX_NO_PEOPLE INTO v_max_people FROM APARTMENTS WHERE APARTMENT_ID = :new.APARTMENT_ID;
...
/*
Trigger checking whether the customer making the reservation does not have too much debt (1000 PLN in this case).
...
DECLARE
    v_balance           number;
    v_max_debt constant number := 1000;
BEGIN
